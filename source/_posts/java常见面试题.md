...
用jstat查看内存使用情况，如果是频繁gc，用jmap把情况dump下来

用jvisualvm查看dump文件，找出占用内存大的对象，并查看其引用情况，查看对应关键代码，是否存在大量写情况。

#### java 8 新特性

函数式编程，lambda表达式，就是把一个函数作为参数传入lambda表达式里面

默认方法：接口可以定义默认方法，实现接口的类可以重写这个默认方法，也可以不重写

重复注解：可以在同一个函数上定义多个相同的注解

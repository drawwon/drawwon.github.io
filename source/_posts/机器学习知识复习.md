...
$$
假设超平面`(w,b)`能够正确分类，也就是对于$y_i=1$，有$w^{T} x_{i}+b>0$；对于$y_i=-1$，有$w^{T} x_{i}+b<0​$，则：
$$
...
5. 常用核函数
   ...
   | 名称       | 表达式                                                       | 参数                                 |
   | ---------- | ------------------------------------------------------------ | ------------------------------------ |
   | 线性核     | $\kappa\left(\boldsymbol{x}_{i}, \boldsymbol{x}_{j}\right)=x_i^Tx_j$ |                                      |
   | 多项式核   | $\kappa\left(\boldsymbol{x}_{i}, \boldsymbol{x}_{j}\right)=(x_i^Tx_j)^d$ | $d \ge1$为多项式的次数               |
   | 高斯核     | $\kappa\left(\boldsymbol{x}_{i}, \boldsymbol{x}_{j}\right)=\exp \left(-\frac{\|x-y\|^{2}}{2\sigma^{2}}\right)$ | $\sigma \ge 0$为高斯核的带宽         |
   | 拉普拉斯核 | $\kappa\left(\boldsymbol{x}_{i}, \boldsymbol{x}_{j}\right)=\exp \left(-\frac{\|x-y\|}{\sigma}\right)$ | $\sigma \ge 0$                       |
   | Sigmoid核  | $\kappa\left(\boldsymbol{x}_{i}, \boldsymbol{x}_{j}\right)=tanh(\beta x_i^Tx_j+\theta)$ | tanh为双正切函数，$\beta>0,\theta<0$ |
